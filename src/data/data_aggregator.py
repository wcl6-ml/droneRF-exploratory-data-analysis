...
Refactor notes (v3):
  - Config loading and path resolution moved inside main() — no side
    effects on import. Importing this module is now safe for tests and
    other scripts that only need the helper functions.
  - build_h5() and build_splits() receive cfg explicitly instead of
    reading module-level globals. Functions are now fully self-contained
    and testable without environment setup.
  - Pure, stateless helpers (read_raw_csv, slice_segments, inspect_h5)
    remain at module level — they carry no config dependency and are safe
    to import freely.
"""
...
def _resolve_paths(cfg: dict) -> tuple[Path, Path, Path, Path]:
    ...
    return root_dir, raw_root, out_file, split_file
 
 
# ---------------------------------------------------------------------------
# Pure helpers  (stateless — safe to import without any env/config setup)
# ---------------------------------------------------------------------------
 
def read_raw_csv(csv_path: Path) -> np.ndarray:
    """Read 1-row × N-column CSV into a 1D float32 array."""
    return np.fromstring(open(csv_path).read(), sep=",", dtype=np.float32)
...
def build_h5(raw_root: Path, out_file: Path, cfg: dict) -> None:
    """
    Write all segments to an HDF5 archive and a companion .meta.parquet.
 
    Parameters
    ----------
    raw_root : Path
        Directory tree containing the raw DroneRF CSV files.
    out_file : Path
    ...
    with h5py.File(out_file, "w") as hf:
        ...
        for bui, drone_type in tqdm(bui_map.items(), desc="BUI"):
            ...
            for csv_path in csv_files:
                ...
                segments = slice_segments(signal, seg_len)
...
def build_splits(meta_parquet: Path, split_file: Path, cfg: dict) -> None:
    """
    ...
    ----------
    meta_parquet : Path
        Companion parquet produced by build_h5().
    split_file : Path
...
def main() -> None:
    ...
    # Importing here keeps these side-effectful calls out of module scope.
    # A test that imports slice_segments never touches .env or the filesystem.
    ...
    build_h5(raw_root, out_file, cfg)
